import './LanguageSelector.css'; // Importa el archivo CSS para el estilo

const LanguageSelector = () => {
  return (
    <div className="language-selector p-20 h-[100vh] flex justify-center items-center flex-col">
            <section className='w-[80%] h-[80%} shadow-2xl p-10 rounded-3xl'>
            <h1 className="mt-6 text-2xl font-bold text-gray-900 sm:text-3xl md:text-4xl">
        Por favor selecciona tu idioma
      </h1>
      <select
        name="languageSelect"
        id="languageSelect"
        className="mt-10 w-full rounded-lg border border-blue-200 bg-transparent p-10 text-gray-700 sm:text-sm h-[100px]"
      >
        <option value="">Selecciona un idioma</option>
        <option value="es">Español</option>
        <option value="en">English</option>
        <option value="fr">Français</option>
      </select>
      {/* Banderas para representar los idiomas */}
      <div className="flags flex mt-20">
        <img src="https://img.asmedia.epimg.net/resizer/Mhe8d93W40EP8OxQ5po5-Pb4-vU=/1472x1104/cloudfront-eu-central-1.images.arcpublishing.com/diarioas/53YSJXSIZFHNTBV52Z4AMKISUM.png" alt="Spanish" className="flag" />
        <img src="data:image/png;base64," alt="English" className="flag" />
        <img src="data:image/png;base64," alt="French" className="flag" />
      </div>
            </section>

    </div>
  );
};

export default LanguageSelector;
